...
const KEY='auditoria-crianza-v4';
const HISTORY_KEY='auditoria-crianza-historial-v1';
...
function snapshotAuditoria(){
  const s=stats();
  return {
    id: state.id || ('AUD-'+Date.now()),
    guardadoEn: new Date().toISOString(),
    datos: JSON.parse(JSON.stringify(state.datos)),
    galpones: JSON.parse(JSON.stringify(activeGalpones().map(({index,...g})=>g))),
    respuestas: JSON.parse(JSON.stringify(state.respuestas)),
    resultado:{cumple:s.c,noCumple:s.n,noAplica:s.na,pendientes:s.p,porcentaje:Number(s.pct.toFixed(2))}
  }
}
function guardarEnHistorial(){
  if(!validateDatos())return;
  const snap=snapshotAuditoria();
  state.id=snap.id; save();
  const list=getHistory();
  const pos=list.findIndex(x=>x.id===snap.id);
  if(pos>=0)list[pos]=snap; else list.unshift(snap);
  saveHistory(list);
  alert('Auditoría guardada en el historial local.');
  renderHistorial();
}
function cargarAuditoria(id){
  const x=getHistory().find(a=>a.id===id); if(!x)return;
  state={id:x.id,datos:x.datos,galpones:x.galpones,respuestas:x.respuestas||{}};
  while(state.galpones.length<3)state.galpones.push(EMPTY_GALPON());
  save(); loadDatos(); renderList(); renderSummary(); tab('datos');
}
function eliminarAuditoria(id){
  if(!confirm('¿Eliminar esta auditoría del historial local?'))return;
  saveHistory(getHistory().filter(x=>x.id!==id)); renderHistorial();
}
function renderHistorial(){
  const box=$('historialLista'); if(!box)return;
  const q=($('buscarHistorial')?.value||'').toLowerCase();
  const list=getHistory().filter(a=>JSON.stringify(a.datos).toLowerCase().includes(q));
  box.innerHTML=list.length?list.map(a=>`<article class="history-card"><div><h3>${esc(a.datos.granja||'Sin granja')}</h3><p><b>Fecha:</b> ${esc(a.datos.fecha)} · <b>Campaña:</b> ${esc(a.datos.campana)} · <b>Visita:</b> ${esc(a.datos.visita)}</p><p><b>Supervisor:</b> ${esc(a.datos.supervisor)} · <b>Auditor(a):</b> ${esc(a.datos.auditor)}</p><p><b>Galpones:</b> ${(a.galpones||[]).map(g=>esc(g.numero)).join(', ')} · <b>Resultado:</b> ${Number(a.resultado?.porcentaje||0).toFixed(1)}%</p></div><div class="history-actions"><button onclick="cargarAuditoria('${a.id}')">Abrir</button><button class="danger" onclick="eliminarAuditoria('${a.id}')">Eliminar</button></div></article>`).join(''):'<p>No hay auditorías guardadas todavía.</p>';
}
function exportarTodoHistorial(){
  const b=new Blob([JSON.stringify(getHistory(),null,2)],{type:'application/json'}),a=document.createElement('a');
  a.href=URL.createObjectURL(b);a.download='historial_auditorias.json';a.click();URL.revokeObjectURL(a.href)
}
...
$('guardarHistorial').onclick=guardarEnHistorial;
...
$('buscarHistorial').oninput=renderHistorial;
$('exportarHistorial').onclick=exportarTodoHistorial;
...
window.addEventListener('online',network);window.addEventListener('offline',network);network();loadDatos();renderList();renderSummary();renderHistorial();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js');
